...
const MasterPosition = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      fetchPositions(form.getFieldsValue());
    } catch (error) {
  ...
  const handleModalSubmit = async () => {
    try {
      ...
      if (editingRecord) {
        await updatePosition(editingRecord.id, values);
  ...
  return (
    <div>
        ...
        >
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <Form.Item name="name" label="Name">
              <Input placeholder="Search by position name" />
            </Form.Item>
      ...
      {/* Add/Edit Modal */}
      <Modal
        title={editingRecord ? 'Edit Position' : 'Add New Position'}
        open={modalVisible}
